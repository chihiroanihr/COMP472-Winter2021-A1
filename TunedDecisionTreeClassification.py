...
def tuning(document, label):
    ...
    pipe = Pipeline(steps=[('std_slc', std_slc),
                           ('pca', pca),
                           ('tree', classifier)])
    n_components = list(range(1, document.shape[1] + 1, 1))
    ...
    clf_GS.fit(document, label)

    print('Best Criterion:', clf_GS.best_estimator_.get_params()['tree__criterion'])
    print('Best max_depth:', clf_GS.best_estimator_.get_params()['tree__max_depth'])
    print('Best Number Of Components:', clf_GS.best_estimator_.get_params()['pca__n_components'])
    print()

    new_classifier = clf_GS.best_estimator_.get_params()['tree']
    return new_classifier


def train_nb(documents, labels, classifier):
...
##### 3. Better (Tuned) Decision Tree #####
print("==================TRANING==================")
# START FROM TUNING
new_classifier = tuning(train_docs_vector, train_labels)
classifier = train_nb(train_docs_vector, train_labels, new_classifier)  # classifier.fit(X_train, y_train)
...
# OUTPUT
with open('Tuned-Best-Decision-Tree-database.txt', 'w') as file:
    file.write("====Classification Used: Tuned Best Decision Tree====\n\n\n")
